...
2. Create a Coolify team API token with read-only permissions. Set `COOLIFY_API_URL` to a complete URL such as `https://coolify.example.com` (the application appends `/api/v1` automatically), then set `COOLIFY_API_KEY`, `APP_ORIGIN`, a random `SESSION_SECRET` of at least 24 characters, and `OWNER_CONTACT_URL`. Keep secrets runtime-only. Coolify recognizes the `:?` declarations in `compose.yaml` and will block a deploy if a required value is empty.
