...
export default function AdminPage() {
  ...
  if (!session.authenticated) return <AdminCentered><div className="admin-login-mark">IZ</div><span className="eyebrow">Owner workspace</span><h1>Izbri Projects admin</h1><p>Sign in with the temporary owner credentials to import, curate, and monitor projects.</p>{session.methods.password && <PasswordLogin />}{session.methods.password && session.methods.github && <span className="login-divider">or</span>}{session.methods.github && <form action="/api/v1/auth/github" method="get"><button className="secondary-button admin-login-button" type="submit"><Github size={19} /> Continue with GitHub</button></form>}{!session.configured && <div className="admin-warning">Admin authentication is not configured. Add ADMIN_USERNAME and ADMIN_PASSWORD to the backend environment.</div>}<Link className="text-link" href="/en">Return to public dashboard</Link></AdminCentered>;
